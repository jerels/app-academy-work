...
VALUES
    ...
    (13, 'teaspoons'),
    (14, '');
SELECT pg_catalog.setval(pg_get_serial_sequence('units_of_measure', 'id'), (SELECT MAX(id)
    FROM units_of_measure)+1);
...
SELECT pg_catalog.setval(pg_get_serial_sequence('recipes', 'id'), (SELECT MAX(id)
    FROM recipes)+1);

INSERT INTO ingredients
    (recipe_id, amount, unit_of_measure_id, food_stuff)
VALUES
...
SELECT pg_catalog.setval(pg_get_serial_sequence('ingredients', 'id'), (SELECT MAX(id)
    FROM ingredients)+1);

INSERT INTO instructions
    (recipe_id, list_order, specification)
VALUES
...
SELECT pg_catalog.setval(pg_get_serial_sequence('instructions', 'id'), (SELECT MAX(id)
    FROM instructions)+1);
